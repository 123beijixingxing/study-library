module.exports = {
  ...
  env: {
    ...
    es2021: true, // 预定义的全局变量，这里是ECMAScript 2021环境
  },
  // 扩展风格
  ...
  rules: {
    ...
    'vue/multi-word-component-names': 'off', // 禁止使用单个单词作为组件名
    'no-new': 'off', // 禁止使用 new 操作符
    'no-useless-return': 'off', // 禁止无用的return
    'vue/no-multiple-template-root': 'off', // 禁止有多个根元素
    'vue/no-v-model-argument': 0, // 禁止在 v-model 中使用修饰符
    'vue/no-mutating-props': 0, // 禁止在props中修改属性值
    'vue/no-v-for-template-key': 'off', // 禁止在 v-for 中使用 v-if
    // 设置每行最大属性数
